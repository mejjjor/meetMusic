...
riot.tag2('mm-player', '<audio id="mp3Player" ontimeupdate="{timeUpdate}" onplaying="{playAudio}"></audio> <ol id="playlist"> <li each="{playlist}"> <mm-item content="{item}"></mm-item> </li> </ol>', '', 'ondragover="{dragover}" ondrop="{drop}"', function (opts) {
    ...
    this.on('mount', function () {
        ...
        var sortable = Sortable.create(document.getElementById('playlist'), {
            ...
            onEnd: function onEnd(evt) {
                var el = _this.playlist[evt.oldIndex];
                _this.playlist.splice(evt.newIndex, 0, _this.playlist.splice(evt.oldIndex, 1)[0]);
                opts.eventBus.trigger('updatePlaylist', _this.playlist);
            }
        });
    });
    ...
    opts.eventBus.on('addFileToItem', function (file, data) {
        ...
        for (var i = 0; i < _this.playlist.length; i++) {
            if (_this.playlist[i].item.file.name == data.name) opts.eventBus.trigger('addMp3Functions', _this.playlist[i].item, URL.createObjectURL(file));
            console.log(_this.playlist[i]);
        }
    });
    ...
    opts.eventBus.on('stopOthers', function (id) {
        for (var i = 0; i < _this.playlist.length; i++) {
            if (_this.playlist[i].item.id != id) {
                if (_this.playlist[i].item.status.play === 'hide') {
                    _this.playlist[i].item.pause();
                    _this.playlist[i].item.status.play = 'show';
                    _this.playlist[i].item.status.pause = 'hide';
                }
            } else {
                _this.playlist[i].item.status.play = 'hide';
                _this.playlist[i].item.status.pause = 'show';
            }
        }
    });
    ...
    opts.eventBus.on('playNext', function () {
        var found = false;
        for (var i = 0; i < _this.playlist.length - 1; i++) {
            if (_this.playlist[i].item.id === _this.currentId) {
                opts.eventBus.trigger('stopOthers', _this.playlist[i + 1].item.id);
                _this.playlist[i + 1].item.play();

                _this.currentId = _this.playlist[i + 1].item.id;
                found = true;
                break;
            }
        }
    ...
    opts.eventBus.on('getSeekTime', function (value) {
        for (var i = 0; i < _this.playlist.length; i++) {
            if (_this.playlist[i].item.id == _this.currentId) {
                _this.playlist[i].item.track.progress = Math.round(value);
                break;
            }
        }_this.update();
    });
    ...
    opts.eventBus.on('setPlaylist', function (playlist) {
        ...
        opts.eventBus.trigger('addRemoteFunctions', _this.playlist);
        _this.update();
    });

    opts.eventBus.on('playId', function (id) {
        for (var _iterator = _this.playlist, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
            var _ref;
    ...
    var getCurrentItem = function getCurrentItem() {
        for (var i = 0; i < _this.playlist.length; i++) {
            if (_this.playlist[i].item.id === _this.currentId) return _this.playlist[i].item;
        }return {};
    };
}, '{ }');
...
riot.tag2('mm-search', '<div id="search" type="text" onpaste="{edit}" onkeyup="{edit}"> <mm-addfile></mm-addFile> </div> <ul> <li each="{results}"> <mm-result content="{item}"></mm-result> </li> </ul>', '', 'ondragover="{dragover}" ondrop="{drop}"', function (opts) {
    ...
    this.on('mount', function () {
        suggest = Completely.completely(_this3.search, {
            fontSize: '24px',
            fontFamily: 'Arial',
            color: '#933'
        });
    ...
    opts.eventBus.on('removeResult', function (name) {
        for (var i = 0; i < _this3.results.length; i++) {
            if (_this3.results[i].item.snippet.title === name) {
                _this3.results.splice(i, 1);
                _this3.update();
                break;
            }
        }
...
riot.tag2('mm-webrtc', '<div> <span>Owner : {isOwner}</span> <button onclick="{reset}">reset</button> </div> <div> <h3>Create a playlist or join one</h3> <input type="text" onkeyup="{edit}" placeholder="playlist name"></input> <button onclick="{createRoom}">Create</button> <button onclick="{joinRoom}">Join</button> </div>', '', '', function (opts) {
    ...
    this.on('mount', function () {
        if (_this5.room) {
            _this5.root.childNodes[2].style.display = 'none';
            webrtc.joinRoom(_this5.room, function (err, res) {
                console.log('joined', _this5.room, err, res);
                isOwner = false;
                global.isOwner = false;
                _this5.isOwner = 'false';
                _this5.update();
            });
        }
    });
    ...
    opts.eventBus.on('addVideo', function (data) {
        data.type = 'video';
        if (isOwner) opts.eventBus.trigger('addItem', _this5.data);else ownerPeer.sendData(data);
    });

    opts.eventBus.on('updatePlaylist', function (playlist) {
        if (isOwner) {
            for (var _iterator2 = peers, _isArray2 = Array.isArray(_iterator2), _i2 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
                var _ref2;
    ...
    opts.eventBus.on('addRemoteFunctions', function (playlist) {
        for (var _iterator3 = playlist, _isArray3 = Array.isArray(_iterator3), _i3 = 0, _iterator3 = _isArray3 ? _iterator3 : _iterator3[Symbol.iterator]();;) {
            var _ref3;
