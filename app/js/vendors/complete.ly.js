...
module.exports = {
    completely: function(container, config) {
        config = config || {};
        ...
        var createDropDownController = function(elem) {
            ...
            var p = {
                ...
                move: function(step) { // moves the selection either up or down (unless it's not possible) step is either +1 or -1.
                    ...
                    if (ix + step === -2 || ix + step === rows.length) return rows[ix].__hint; // NO CIRCULAR SCROLLING. 
                    ix += step;
                    p.highlight(ix);
                    if (ix != -1)
        ...
        var keyDownHandler = function(e) {
            ...
            if (keyCode == 13) { // enter  (autocomplete triggered)
                console.log('eeeee')
                if (txtHint.value.length == 0 && dropDownController.isOptionsSelected()) { // if there is a hint
                    rs.onEnter();
                } else {
                    var wasDropDownHidden = (dropDown.style.visibility == 'hidden');
                    dropDownController.hide();
                    if (wasDropDownHidden && dropDownController.isOptionsSelected()) {
                        txtHint.value = txtInput.value; // ensure that no hint is left.
                        txtInput.focus();
                        rs.onEnter();
                        return;
                    }
                    if (dropDownController.isOptionsSelected()) {
                        txtInput.value = txtHint.value;
                        var hasTextChanged = registerOnTextChangeOldValue != txtInput.value
                        registerOnTextChangeOldValue = txtInput.value; // <-- to avoid dropDown to appear again. 
                        // for example imagine the array contains the following words: bee, beef, beetroot
